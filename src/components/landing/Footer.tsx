...
export function Footer() {
  return (
    <footer className="relative bg-cosmos overflow-hidden" style={{ borderTop: "1px solid var(--border-1)" }}>
      <div className={`${LANDING_GUTTERS} py-24 md:py-28 lg:py-32 relative`}>
        <div className="flex justify-center gap-4 mb-16 md:mb-20">
        ...
        <div
          className="flex flex-col items-center justify-center gap-4 pt-10 md:pt-12 text-center"
          style={{ borderTop: "1px solid var(--border-1)" }}
        >
          <p className="font-mono text-[10px] text-haze-3">© 2026 Bountixx. All rights reserved.</p>
          <p className="text-xs text-haze-3">Built for champions</p>
        </div>
      </div>
    </footer>
  );
...
function SocialIcon({ label, href, children }: { label: string; href: string; children: React.ReactNode }) {
  return (
    <a
      ...
      style={{ border: "1px solid var(--border-1)", color: "var(--haze-3)" }}
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLElement).style.color = "var(--haze)";
        (e.currentTarget as HTMLElement).style.borderColor = "var(--haze-3)";
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLElement).style.color = "var(--haze-3)";
