    ...
    <footer className="bg-cosmos border-t border-cosmos-4">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-14 py-16">
        <div className="grid grid-cols-1 md:grid-cols-[auto_1fr_auto] gap-12 items-start mb-12">
          ...
          <nav className="flex flex-wrap gap-x-8 gap-y-3 md:justify-center md:pt-3">
            {NAV_LINKS.map(({ label, href }) => (
              <Link
                key={label}
                href={href}
                className="cursor-target font-rajdhani text-sm text-haze-2 hover:text-haze transition-colors"
              >
                {label}
              </Link>
            ))}
          ...
          {/* Socials — proper SVG icons */}
          <div className="flex gap-3 md:pt-3">
            {/* Twitter/X */}
            <a
              href="#"
              aria-label="Twitter"
              className="cursor-target w-9 h-9 flex items-center justify-center border border-cosmos-4 text-haze-3 hover:text-haze hover:border-void/50 transition-all"
            >
