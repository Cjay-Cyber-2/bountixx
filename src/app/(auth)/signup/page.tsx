...
export default function SignupPage() {
  ...
  async function verifyEmailForPassword(e: React.FormEvent) {
    e.preventDefault();
    if (!isLoaded) return;
    setError("");
    setPending(true);
  ...
  async function handleEmailOtp(e: React.FormEvent) {
    ...
    setPending(true);
    try {
      await signUp.create({ username, emailAddress: email });
      await signUp.prepareEmailAddressVerification({ strategy: "email_code" });
      setVerifying(true);
    } catch (err: unknown) {
      const clerkErr = err as { errors?: { message: string }[] };
  ...
  async function handlePhoneOtp(e: React.FormEvent) {
    ...
    setPending(true);
    try {
      await signUp.create({ username, phoneNumber: phone });
    ...
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2 bg-cosmos">
      ...
      <div className="flex items-center justify-center px-6 py-16 bg-cosmos">
        ...
        >
            ...
            <motion.div variants={slideUp} className="flex mb-6 border border-cosmos-4">
              {methodTabs.map((tab) => (
                <button
                  key={tab.id}
                  type="button"
                  onClick={() => { setMethod(tab.id); setError(""); setOtp(""); }}
          ...
          {/* Email + Password */}
          {method === "email-password" && !verifying && (
            <motion.form key="ep" variants={slideUp} initial="hidden" animate="show" onSubmit={handleEmailPassword} className="flex flex-col gap-5">
              <Field label="USERNAME" type="text" placeholder="arena_name" value={username} onChange={setUsername} />
              <Field label="EMAIL ADDRESS" type="email" placeholder="you@arena.com" value={email} onChange={setEmail} />
              <PasswordField label="PASSWORD" show={show} onToggle={() => setShow(!show)} value={password} onChange={setPassword} />
              {error && <p className="font-rajdhani text-sm text-red-400">{error}</p>}
              <Button variant="primary" size="lg" fullWidth magnetic className="mt-2" disabled={pending}>
                {pending ? "CREATING…" : "ENTER THE ARENA"}
              </Button>
            </motion.form>
          )}

          {method === "email-password" && verifying && (
            <motion.form key="ep-verify" variants={slideUp} initial="hidden" animate="show" onSubmit={verifyEmailForPassword} className="flex flex-col gap-5">
              <p className="font-rajdhani text-sm text-haze-2">We sent a verification code to <span className="text-void">{email}</span>.</p>
              <div>
              ...
              <Button variant="primary" size="lg" fullWidth magnetic disabled={pending}>
                {pending ? "VERIFYING…" : "VERIFY & JOIN"}
              </Button>
            </motion.form>
          )}

          {/* Email OTP */}
          {method === "email-otp" && !verifying && (
            <motion.form key="eotp" variants={slideUp} initial="hidden" animate="show" onSubmit={handleEmailOtp} className="flex flex-col gap-5">
              <Field label="USERNAME" type="text" placeholder="arena_name" value={username} onChange={setUsername} />
              <Field label="EMAIL ADDRESS" type="email" placeholder="you@arena.com" value={email} onChange={setEmail} />
              {error && <p className="font-rajdhani text-sm text-red-400">{error}</p>}
              <Button variant="primary" size="lg" fullWidth magnetic className="mt-2" disabled={pending}>
          ...
          {method === "email-otp" && verifying && (
            <motion.form key="eotp-verify" variants={slideUp} initial="hidden" animate="show" onSubmit={verifyEmailOtp} className="flex flex-col gap-5">
              <p className="font-rajdhani text-sm text-haze-2">We sent a code to <span className="text-void">{email}</span>.</p>
              <div>
          ...
          {/* Phone OTP */}
          {method === "phone-otp" && !verifying && (
            <motion.form key="potp" variants={slideUp} initial="hidden" animate="show" onSubmit={handlePhoneOtp} className="flex flex-col gap-5">
              <Field label="USERNAME" type="text" placeholder="arena_name" value={username} onChange={setUsername} />
              <div>
          ...
          {method === "phone-otp" && verifying && (
            <motion.form key="potp-verify" variants={slideUp} initial="hidden" animate="show" onSubmit={verifyPhoneOtp} className="flex flex-col gap-5">
              <p className="font-rajdhani text-sm text-haze-2">We sent a code to <span className="text-void">{phone}</span>.</p>
              <div>
                <label className="block font-space-mono text-[11px] text-void tracking-[3px] mb-2 uppercase">Enter Code</label>
                <input
                  type="text"
          ...
          <motion.p variants={slideUp} className="font-rajdhani text-sm text-haze-3 text-center mt-8">
            ...
            <Link href="/login" className="cursor-target text-void hover:underline">Sign in</Link>
          </motion.p>
        </motion.div>
      </div>
    </div>
...
function Field({ label, type, placeholder, value, onChange }: {
  ...
  return (
    <div>
      <label className="block font-space-mono text-[11px] text-void tracking-[3px] mb-2 uppercase">{label}</label>
      <input
        type={type}
